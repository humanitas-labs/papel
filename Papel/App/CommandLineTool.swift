...
/// The `papel` command: a symlink on the PATH to the launcher script in
...
final class CommandLineTool: ObservableObject {
    static let shared = CommandLineTool(
        launcher: Bundle.main.resourceURL!.appendingPathComponent("papel"))

    enum Status: Equatable {
        /// No `papel` on the PATH.
        case notInstalled
        ...
        /// A `papel` on the PATH that is not a link to a Papel bundle; left alone.
        case foreign(URL)
    ...
    /// The first `papel` on the PATH, following `which`.
    ...
    /// The PATH of the user's login shell, which is where `papel` has to be
