...
struct CommandLineToolTests {
    private struct Harness {
        ...
        init() throws {
            ...
            launcher = root.appendingPathComponent("Paper.app/Contents/Resources/paper")
        ...
        var link: URL { bin.appendingPathComponent("paper") }
    }
    ...
    func classifiesLinks() throws {
        ...
        let other = h.root.appendingPathComponent("Elsewhere/Paper.app/Contents/Resources/paper")
    ...
    func installWritesRepairsAndUninstallRemoves() async throws {
        ...
        try await Self.withoutOptOut {
            ...
            try FileManager.default.createSymbolicLink(at: h.link, withDestinationURL: h.root.appendingPathComponent("Old.app/Contents/Resources/paper"))
            await tool.refresh()
    ...
    func quietInstallRepairsWritableLinksAndDeclinesUnwritableOnes() async throws {
        ...
        try await Self.withoutOptOut {
            ...
            let other = h.root.appendingPathComponent("Other.app/Contents/Resources/paper")
            try FileManager.default.createDirectory(at: other.deletingLastPathComponent(), withIntermediateDirectories: true)
            try "#!/bin/sh\n".write(to: other, atomically: true, encoding: .utf8)
            try FileManager.default.removeItem(at: h.link)
            try FileManager.default.createSymbolicLink(at: h.link, withDestinationURL: other)
    ...
    func quotesShellArguments() {
        #expect(CommandLineTool.quoted("/Applications/Paper.app") == "'/Applications/Paper.app'")
