...
- Six heading levels are six sizes. `#` through `######` sit at 12, 8, 5, 3, 1, and 0 points above the body, 26 / 22 / 19 / 17 / 15 / 14 at the default; before, the step was 2 points with a floor that left `####`, `#####`, and `######` identical. `######` is body-sized and takes the quote ink so it still reads as a heading (#38).
