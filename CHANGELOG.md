...
- Papel tells you when a newer release is out. Once a day on launch it asks GitHub for the latest release, and when that is newer than the running version the welcome window shows a download arrow at its bottom left, in the theme's accent, the one coloured tone on the page (`color.accent`, teal in Enso); hovering names the version and makes it bob, and a click opens the DMG link. Nothing shows when the app is current, offline, or the request fails, and the check never delays launch. `update.check = off` in the config, or the switch under Settings ▸ Updates, turns it off; off never makes the request (#45).
