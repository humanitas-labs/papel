...
- Task items sit closer to the margin than bullets: the circle is indented 10 points by default instead of a bullet's 21, since the wide circle otherwise pushes the text further in than a dash does. `task.indent` in the config, and a Task indent slider under Settings ▸ Lists, set it; 0 puts the circle on the margin. Nested tasks step in from there as nested bullets do.
