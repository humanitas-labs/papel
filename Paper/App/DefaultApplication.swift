...
/// Paper as the app a double-click on a Markdown file opens: what Get
/// Info's Change All does, set from code instead. `paper --set-default`
...
enum DefaultApplication {
    ...
    /// Whether Paper is what opens Markdown files now. Launch Services keeps
    ...
    /// Makes this copy of Paper the default for Markdown files.
    @MainActor
    ...
    static func runFromCommandLine() -> Never {
        if isDefault {
            print("Paper already opens Markdown files.")
            exit(0)
        }
        ...
        switch outcome {
        case .success, .none:
            print("Paper now opens Markdown files (.md, .markdown).")
            exit(0)
        case .failure(let error):
            FileHandle.standardError.write(Data("paper: could not make Paper the default app for Markdown: \(error.localizedDescription)\n".utf8))
            exit(1)
        }
    }
}
