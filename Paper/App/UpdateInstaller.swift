...
enum UpdateInstaller {
    ...
    static func install(
        ...
        let scratch = FileManager.default.temporaryDirectory
            .appendingPathComponent("paper-update-\(release.version)-\(UUID().uuidString)", isDirectory: true)
        ...
        let dmg = scratch.appendingPathComponent("Paper.dmg")
        ...
        guard let mounted = app(inside: mount) else { throw Failure.noApp }
    ...
    /// The one `.app` on the image, whatever it is called; the name of
    /// the running copy is not what the release is checked against.
    static func app(inside mount: URL) -> URL? {
    ...
    private static func download(
        ...
        request.setValue("Paper/\(UpdateCheck.bundleVersion)", forHTTPHeaderField: "User-Agent")
    ...
    static func requirementString(for installed: URL) throws -> String {
        let identifier = Bundle(url: installed)?.bundleIdentifier ?? "org.humanitas.paper"
