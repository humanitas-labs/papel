...
This record documents the existing implementation. `PapelLayoutManager` represents concealed punctuation as `.controlCharacter` glyphs and returns `.zeroAdvancement` from its control-character action delegate. It does not use the `.null` mechanism described in ADR-002.
...
The current implementation lives in `Papel/Editor/PapelLayoutManager.swift`; the [architecture overview](../architecture.md) describes the active editor pipeline.
